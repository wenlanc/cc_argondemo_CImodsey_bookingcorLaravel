...
<?php function print_sub_categories($categories, $category_ids, $selected_categories, $selected_products)
{
    $ci =& get_instance();
    $html = '<ul>';
...
<div class="row">
    ...
    <div class="col-sm-12">
        <div class="box">
            ...
            <div class="box-body">
                <?php echo form_open("edit-coupon-post"); ?>
                <input type="hidden" name="id" value="<?= $coupon->id; ?>">
                <div class="form-group">
                    <label class="control-label"><?php echo trans("coupon_code"); ?>&nbsp;&nbsp;<small>(<?= trans("exp_special_characters"); ?> E.g: #, *, % ..)</small></label>
                    <div class="position-relative">
                        <input type="text" name="coupon_code" id="input_coupon_code" value="<?= html_escape($coupon->coupon_code); ?>" class="form-control form-input" placeholder="<?php echo trans("coupon_code"); ?>" maxlength="49" required>
                ...
                <div class="form-group">
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" name="discount_rate" id="input_discount_rate" value="<?= $coupon->discount_rate; ?>" aria-describedby="basic-addon-discount" class="form-control form-input" placeholder="E.g: 5" min="0" max="99" required>
                    </div>
                </div>
                <div class="form-group">
                    <label class="control-label"><?php echo trans("number_of_coupons"); ?>&nbsp;<small>(<?= trans("number_of_coupons_exp"); ?>)</small></label>
                    <input type="number" name="coupon_count" value="<?= $coupon->coupon_count; ?>" class="form-control form-input" placeholder="E.g: 100" min="1" max="99999999" required>
                </div>
                <div class="form-group">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" name="minimum_order_amount" id="product_price_input" value="<?= $coupon->minimum_order_amount != 0 ? get_price($coupon->minimum_order_amount, 'input') : ''; ?>" aria-describedby="basic-addon1" class="form-control form-input price-input validate-price-input" placeholder="<?php echo $this->input_initial_price; ?>" onpaste="return false;" maxlength="32">
                    </div>
                </div>
                <div class="form-group">
                    <div class="row">
                    ...
                    <div class="row">
                        <div class="col-sm-12 max-600">
                            ...
                            <div class='input-group date' id='datetimepicker'>
                                <input type='text' class="form-control" name="expiry_date" value="<?= html_escape($coupon->expiry_date); ?>" placeholder="<?php echo trans("expiry_date"); ?>" required>
                                            ...
                                            if (!empty($products)):?>
                                                <div class="items">
                                                    <?php foreach ($products as $product): ?>
                                                        <div class="item">
                                                            <input type='checkbox' name='product_id[]' id="cb_product_<?= $product->id; ?>" value='<?= $product->id; ?>' <?= in_array($product->id, $selected_products) ? 'checked' : ''; ?> data-parent="<?= $category->id; ?>">&nbsp;&nbsp;<label for="cb_product_<?= $product->id; ?>"><?= html_escape($product->title); ?></label>
                                                        </div>
                                                    <?php endforeach; ?>
                                                </div>
                                            <?php endif; ?>
