...
class Earnings_model extends CI_Model
{
    ...
    public function add_seller_earnings($order_product)
    {
        if (!empty($order_product)) {
            ...
            if (!empty($order)) {
                ...
                $query = $this->db->where('order_number', $order->order_number)->where('order_product_id', $order_product->id)->where('user_id', $order_product->seller_id)->get('earnings');
                $num_rows = $query->num_rows();
                if ($num_rows < 1) {
                    $sale_amount = get_price($order_product->product_total_price, 'decimal');
                    $vat = 0;
                    if (!empty($order_product->product_vat)) {
                        $vat = get_price($order_product->product_vat, 'decimal');
                    }
                    $sale_amount = $sale_amount - $vat;
                    //calculate earned amount
                    $earned_amount = $sale_amount;
                    $coupon_discount = 0;
                    $product_ids = get_coupon_products_array($order);
                    if (!empty($product_ids) && in_array($order_product->product_id, $product_ids)) {
                        if ($order->coupon_discount_rate > 0 && $order->coupon_seller_id == $order_product->seller_id) {
                            $coupon_discount = ($sale_amount * $order->coupon_discount_rate) / 100;
                            $earned_amount = $earned_amount - $coupon_discount;
                        }
                    }
                    $commission = ($sale_amount * $order_product->commission_rate) / 100;
                    $earned_amount = $earned_amount - $commission;
                    ...
                    $earned_amount = $earned_amount + $vat;
                    //add earning
                    $data = array(
                        'order_number' => $order->order_number,
                        'order_product_id' => $order_product->id,
                        'user_id' => $order_product->seller_id,
                        'sale_amount' => get_price($sale_amount, 'database'),
                        'vat_rate' => $order_product->product_vat_rate,
                        'vat_amount' => $order_product->product_vat,
                        'commission_rate' => $order_product->commission_rate,
                        'commission' => get_price($commission, 'database'),
                        'coupon_discount' => get_price($coupon_discount, 'database'),
                        'shipping_cost' => get_price($shipping_cost, 'database'),
                        'earned_amount' => get_price($earned_amount, 'database'),
    ...
    public function get_single_product_shipping_cost($order_id, $seller_id)
    {
        ...
        if (!empty($num_products)) {
            $cost = ($seller_shipping_cost / 100) / $num_products;
