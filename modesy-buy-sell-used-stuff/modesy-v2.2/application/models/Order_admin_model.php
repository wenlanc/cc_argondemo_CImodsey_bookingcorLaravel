...
class Order_admin_model extends CI_Model
{
    ...
    public function update_order_status_if_completed($order_id)
    {
        ...
        if (!empty($order_products)) {
            foreach ($order_products as $order_product) {
                if ($order_product->order_status == "awaiting_payment" || $order_product->order_status == "payment_received" || $order_product->order_status == "order_processing" || $order_product->order_status == "shipped") {
                    $all_complated = false;
                }
            }
            $data = array(
